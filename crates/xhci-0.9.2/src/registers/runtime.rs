...
impl EventRingDequeuePointerRegister {
    ...
    /// Sets the Event Handler Busy bit to 1 (which should trigger hardware to clear it).
    /// According to XHCI spec, writing 1 to EHB while updating ERDP should clear the bit.
    pub fn set_event_handler_busy(&mut self) {
        self.0 |= 1 << 3;
    }
}
impl_debug_from_methods! {
