    ...
    ($name:ident($ty_expected:expr) {
        ...
        impl TryFrom<[u32; 4]> for $name{
            ...
            fn try_from(raw:[u32;4])->Result<Self,Self::Error>{
                ...
                trace!(
                    "[xhci::trb::{}::TryFrom] Raw: [{:#010X}, {:#010X}, {:#010X}, {:#010X}]",
                    stringify!($name),
                ...
                $(if raw[$index].get_bits($range) != 0{
                    trace!(
                        "[xhci::trb::{}::TryFrom] Reserved bit check failed: index {}, range {:?}, value {:#X}",
                        stringify!($name), $index, $range, raw[$index].get_bits($range)
                    );
                    return Err(raw);
                })*

                let incoming_type_val = raw[3].get_bits(10..=15);
                let expected_type_val = $ty_expected as u32;
                trace!(
                    "[xhci::trb::{}::TryFrom] Type check: Incoming ID {:#X} ({}), Expected ID {:#X} ({})",
                    stringify!($name),
                    incoming_type_val, incoming_type_val, expected_type_val, expected_type_val
                );

                if incoming_type_val != expected_type_val {
                    trace!(
                        "[xhci::trb::{}::TryFrom] Type mismatch: Incoming {:#X} != Expected {:#X}. Returning Err.",
                        stringify!($name), incoming_type_val, expected_type_val
                    );
                    return Err(raw);
                }

                trace!(
                    "[xhci::trb::{}::TryFrom] Successful. Returning Ok(Self(raw)).",
                    stringify!($name)
                );
                Ok(Self(raw))
            }
